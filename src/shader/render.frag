...
void main() {
    ...
    vec3 r = calcRay(c_pos, c_target, c_up, c_fov,
                     c_res, gl_FragCoord.xy);
    vec3 pc = (u_rasterToCamera * vec4(gl_FragCoord.xy, 0, 1)).xyz;
    outColor = vec4((pc), 1);//vec4(((u_cameraToWorld * vec4(normalize(pc), 1)).xyz), 1);
    //    outColor = vec4( gl_FragCoord.xy / c_res.xy, 0, 1.0 );
    //    outColor = vec4(rayDir,1);
    outColor = gammaCorrect(vec4(computeColor(rayOrg, rayDir), 1));
}
